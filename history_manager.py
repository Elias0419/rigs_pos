...
class HistoryView(BoxLayout):
    ...
    def _build_totals_bar(self):
        ...
        self.current_filter_label = MDLabel(text="Current Filter: today", size_hint_x=0.8, markup=True)
        ...
        for w in (self.history_search, blank, self.current_filter_label,
                  self.average_label, self.total_cash_label, self.total_amount_label):
    ...
    def update_totals(self):
        if not self.rv_data:
            ...
            self.total_amount_label.text = "[size=20]Total: 0.00 + 0.00 tax = \n[b]$0.00[/b][/size]"
            self.total_cash_label.text   = "[size=20]Cash: 0.00 - 0.00 change = \n[b]$0.00[/b][/size]"
        ...
        if self.current_filter == "custom_range":
            ...
            if first and last:
                self.current_filter_label.text = f"Range: {first.strftime('%m/%d/%Y')} - {last.strftime('%m/%d/%Y')}"
            else:
    ...
    def is_today(self, dtobj):        return dtobj.date() == datetime.today().date()
    def is_yesterday(self, dtobj):    return dtobj.date() == (datetime.today() - timedelta(days=1)).date()
    ...
    def show_order_details(self, order_id):
        specific = next((o for o in self.order_history if str(o[0]) == str(order_id)), None)
        if specific:
            self.clear_widgets()
            try:
    ...
    def format_date(self, s):
        try:
            return datetime.strptime(s, "%Y-%m-%d %H:%M:%S.%f").strftime("%d %b %Y, %H:%M")
        except Exception as e:
            logger.warning(e)
            return s
    ...
    def export_history(self, *_):
        ...
        with open(filename, "w", newline="", encoding="utf-8") as f:
            w = csv.writer(f)
            w.writerow(["Order ID","Items","Total","Tax","Discount","Total with Tax",
                        "Timestamp","Payment Method","Amount Tendered","Change Given"])
            w.writerows(rows)
    ...
    def get_export_filename(self):
        ...
        if m == "today":         return f"Order_History_Today_{today}.csv"
        if m == "this_week":     return f"Order_History_This_Week_{today}.csv"
        if m == "this_month":    return f"Order_History_This_Month_{today}.csv"
        if m == "custom_range":  return f"Order_History_Custom_Range_{today}.csv"
        if m == "specific_day":  return f"Order_History_Specific_Day_{today}.csv"
        return f"Order_History_All_{today}.csv"

    def prepare_csv_data(self):
        return [[o.get("order_id"), o.get("items"), o.get("total"), o.get("tax"),
                 o.get("discount"), o.get("total_with_tax"), o.get("timestamp"),
                 o.get("payment_method"), o.get("amount_tendered"), o.get("change_given")]
...
class HistoryPopup(Popup):
    ...
    def __init__(self, **kwargs):
        ...
        self._init = True
        self.db_manager = DatabaseManager("db/inventory.db", self)
        self.history_view = HistoryView()  # keep a single view to preserve filters between opens
...
class OrderDetailsPopup(Popup):
    def __init__(self, order, receipt_printer, **kwargs):
        ...
        btns.add_widget(MDRaisedButton(text="Print Receipt",
                                       on_release=lambda *_: self.print_receipt(order)))
        btns.add_widget(MDRaisedButton(text="Refund", on_release=self.refund))
        btns.add_widget(MDRaisedButton(text="Edit",
                                       on_release=lambda *_: self.open_modify_order_popup(order[0])))
        btns.add_widget(MDRaisedButton(text="Close", on_release=lambda *_: self.dismiss()))
    ...
    def convert_order_to_dict(self, order):
        (order_id, items_json, total, tax, discount, total_with_tax,
         timestamp, payment_method, amount_tendered, change_given) = order
        try:
        ...
        items_dict = {str(i): it for i, it in enumerate(items)} if isinstance(items, list) else items
        return {
            "order_id": order_id, "items": items_dict,
            "subtotal": total, "tax_amount": tax, "total_with_tax": total_with_tax,
